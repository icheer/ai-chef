  ...
  const userData = {
    user_profile: {
      ...
      dietary_restrictions: {
        allergies: requestData.dietaryRestrictions.allergies.split(',').map(s => s.trim()).filter(s => s),
        intolerances: requestData.dietaryRestrictions.intolerances.split(',').map(s => s.trim()).filter(s => s),
        dislikes: requestData.dietaryRestrictions.dislikes.split(',').map(s => s.trim()).filter(s => s),
...
function parseGeminiResponse(response) {
  try {
    ...
    if (jsonStart !== -1 && jsonEnd > jsonStart) {
      ...
      if (parsed.recipe_recommendations && parsed.recipe_recommendations.main_recipes && parsed.recipe_recommendations.main_recipes.length > 0) {
